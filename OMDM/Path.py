...
class Cell:
	...
	def find_neighbours(self, grid):
		...
		if (self.row < self.total_size - 1 and self.col > 0) and not grid[self.row + 1][
			self.col - 1].is_wall():  # down left neighbour
			self.neighbours.append(grid[self.row + 1][self.col - 1])
		
		if (self.row < self.total_size - 1 and self.col < self.total_rows - 1) and not grid[self.row + 1][
			self.col + 1].is_wall():  # down right neighbour
			self.neighbours.append(grid[self.row + 1][self.col + 1])
		...
		if (self.row > 0 and self.col < self.total_rows - 1) and not grid[self.row - 1][
			self.col + 1].is_wall():  # upper right neighbour
			self.neighbours.append(grid[self.row - 1][self.col + 1])
